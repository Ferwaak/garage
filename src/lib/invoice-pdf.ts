...
function qrField(value: string | number | null | undefined, maxLength: number) {
  return qrText(value).slice(0, maxLength);
}

function qrIban(value: string | null | undefined) {
  return qrText(value).replace(/[^a-zA-Z0-9]/g, "").toUpperCase();
}

function formatIbanForDisplay(value: string | null | undefined) {
  const account = qrIban(value);
  return account.replace(/(.{4})/g, "$1 ").trim();
}

function qrCurrency(value: string | null | undefined) {
  const currency = qrText(value).toUpperCase();
  return currency === "EUR" ? "EUR" : "CHF";
}

function ibanChecksumIsValid(iban: string) {
...
function isSupportedSwissQrAccount(iban: string) {
  return /^(CH|LI)[0-9A-Z]{19}$/.test(iban) && ibanChecksumIsValid(iban);
}

function isQrIban(iban: string) {
  const iid = Number(iban.slice(4, 9));
  return Number.isInteger(iid) && iid >= 30000 && iid <= 31999;
}

function qrReferenceCheckDigit(baseReference: string) {
  const table = [0, 9, 4, 6, 8, 2, 7, 1, 3, 5];
  let carry = 0;

  for (const digit of baseReference) {
...
function qrReference(invoice: Invoice) {
  const source = `${invoice.invoice_number}${invoice.id}`;
  const numericSource = Array.from(source)
    .map((char) => char.charCodeAt(0).toString().padStart(3, "0"))
    .join("");
  const baseReference = numericSource.padStart(26, "0").slice(-26);
...
function qrPaymentData(
  garage: Garage,
  invoice: Invoice,
  customer: Customer | null
) {
  const account = qrIban(garage.iban);
  ...
  const payload = [
    ...
    account,
    "S",
    qrField(creditorName, 70),
    qrField(creditorAddress.street, 70),
    qrField(creditorAddress.houseNumber, 16),
    qrField(garage.postal_code, 16),
    qrField(garage.city, 35),
...
function drawPaymentSlip(
  ...
  if (paymentData.isValidAccount) {
    ...
    doc.rect(86.5, y + 39.5, 7, 7, "F");
    doc.setFillColor(215, 0, 0);
    doc.rect(89.2, y + 40.4, 1.6, 5.2, "F");
    doc.rect(87.4, y + 42.2, 5.2, 1.6, "F");
  } else {
  ...
  if (referenceDisplay) {
    doc.setFont("helvetica", "bold");
    doc.setFontSize(7);
    doc.text("Reference", 118, y + 46);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(8);
    doc.text(referenceDisplay, 118, y + 51);
  }
  doc.setFont("helvetica", "bold");
  doc.setFontSize(7);
  doc.text("Payable par", 118, referenceDisplay ? y + 62 : y + 58);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(10);
  doc.text(payerLines.slice(1), 118, referenceDisplay ? y + 69 : y + 65);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(8);
